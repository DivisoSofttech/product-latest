...
class SupplierGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Supplier entity")
        ...
        .repeat(2) {
            ...
            .get("/product/api/suppliers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new supplier")
            .post("/product/api/suppliers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "iDPcode":"SAMPLE_TEXT"
                ...
                , "creditLimit":null
                , "currentDebt":null
                , "debtDate":"2020-01-01T00:00:00.000Z"
                , "visible":null
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_supplier_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created supplier")
                .get("/product${new_supplier_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created supplier")
            .delete("/product${new_supplier_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
