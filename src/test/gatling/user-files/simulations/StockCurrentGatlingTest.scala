...
/**
 * Performance test for the StockCurrent entity.
 */
class StockCurrentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the StockCurrent entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all stockCurrents")
            .get("/product/api/stock-currents")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new stockCurrent")
            .post("/product/api/stock-currents")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "iDPcode":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_stockCurrent_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created stockCurrent")
                .get("/product${new_stockCurrent_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created stockCurrent")
            .delete("/product${new_stockCurrent_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
