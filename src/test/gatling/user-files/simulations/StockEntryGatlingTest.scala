...
/**
 * Performance test for the StockEntry entity.
 */
class StockEntryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the StockEntry entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all stockEntries")
            .get("/product/api/stock-entries")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new stockEntry")
            .post("/product/api/stock-entries")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "iDPcode":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_stockEntry_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created stockEntry")
                .get("/product${new_stockEntry_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created stockEntry")
            .delete("/product${new_stockEntry_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
