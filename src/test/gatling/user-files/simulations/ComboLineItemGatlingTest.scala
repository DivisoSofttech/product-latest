...
/**
 * Performance test for the ComboLineItem entity.
 */
class ComboLineItemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ComboLineItem entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all comboLineItems")
            .get("/product/api/combo-line-items")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new comboLineItem")
            .post("/product/api/combo-line-items")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "quantity":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_comboLineItem_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created comboLineItem")
                .get("/product${new_comboLineItem_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created comboLineItem")
            .delete("/product${new_comboLineItem_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
