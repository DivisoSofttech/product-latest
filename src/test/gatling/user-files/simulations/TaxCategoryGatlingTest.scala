...
/**
 * Performance test for the TaxCategory entity.
 */
class TaxCategoryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TaxCategory entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all taxCategories")
            .get("/product/api/tax-categories")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new taxCategory")
            .post("/product/api/tax-categories")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "iDPcode":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_taxCategory_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created taxCategory")
                .get("/product${new_taxCategory_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created taxCategory")
            .delete("/product${new_taxCategory_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
