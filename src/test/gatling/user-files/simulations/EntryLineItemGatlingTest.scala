...
/**
 * Performance test for the EntryLineItem entity.
 */
class EntryLineItemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the EntryLineItem entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all entryLineItems")
            .get("/product/api/entry-line-items")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new entryLineItem")
            .post("/product/api/entry-line-items")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_entryLineItem_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created entryLineItem")
                .get("/product${new_entryLineItem_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created entryLineItem")
            .delete("/product${new_entryLineItem_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
