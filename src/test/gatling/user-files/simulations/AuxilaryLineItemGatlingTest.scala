...
/**
 * Performance test for the AuxilaryLineItem entity.
 */
class AuxilaryLineItemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AuxilaryLineItem entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all auxilaryLineItems")
            .get("/product/api/auxilary-line-items")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new auxilaryLineItem")
            .post("/product/api/auxilary-line-items")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_auxilaryLineItem_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created auxilaryLineItem")
                .get("/product${new_auxilaryLineItem_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created auxilaryLineItem")
            .delete("/product${new_auxilaryLineItem_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
