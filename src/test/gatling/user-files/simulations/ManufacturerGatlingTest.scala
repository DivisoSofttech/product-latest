...
class ManufacturerGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Manufacturer entity")
        ...
        .repeat(2) {
            ...
            .get("/product/api/manufacturers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new manufacturer")
            .post("/product/api/manufacturers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "iDPcode":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_manufacturer_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created manufacturer")
                .get("/product${new_manufacturer_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created manufacturer")
            .delete("/product${new_manufacturer_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
